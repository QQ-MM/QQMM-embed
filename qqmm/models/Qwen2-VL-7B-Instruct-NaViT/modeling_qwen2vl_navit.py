...
class VisionFusionAttention(VisionAttention):

    def forward(
        self, hidden_states: torch.Tensor, cu_seqlens: torch.Tensor, rotary_pos_emb: torch.Tensor
    ) -> torch.Tensor:
        seq_length = hidden_states.size(0)
        q, k, v = self.qkv(hidden_states).view(seq_length, 3, self.num_heads, self.head_dim).unbind(1)
        q, k = apply_rope(q, rotary_pos_emb, 1), apply_rope(k, rotary_pos_emb, 1)
        ...
        attn_output = self.proj(attn_output.flatten(1))
